...
function getRandomFilePath() {
  ...
  let randomIndex, randomLetter, randomSuffix;

  do {
    randomIndex = Math.floor(Math.random() * 12) + 1;
    randomLetter = letters[Math.floor(Math.random() * letters.length)];
    randomSuffix = Math.floor(Math.random() * 2); // 0 or 1
  } while (randomIndex === previousAnswer && randomLetter === previousLetter && randomSuffix === previousSuffix);
  ...
  currentFilePath = `../lyd-intervaller-melodisk/${String(randomIndex).padStart(2, '0')}${randomLetter}${randomSuffix}.opus`;
