...
let currentAnswerSuffix = null;

function getRandomFilePath() {
  let randomIndex, randomSuffix;

  do {
    randomIndex = Math.floor(Math.random() * 16) + 1;
    randomSuffix = Math.floor(Math.random() * 2); // 0 or 1
  } while (randomIndex === previousIndex && randomSuffix === previousSuffix);

  previousIndex = randomIndex;
  previousSuffix = randomSuffix;

  currentAnswerIndex = randomIndex;
  currentAnswerSuffix = randomSuffix;

  return `../lyd-intervaller-melodisk/${String(randomIndex).padStart(2, '0')}g${randomSuffix}.opus`;
}
...
async function playCurrentSound() {
  ...
  let filePath;
  if (currentAnswerIndex === null || currentAnswerSuffix === null) {
    filePath = getRandomFilePath();
    isGuessing = true;
    enableAllLinks(); // Enable guessing links after first play
  } else {
    filePath = `../lyd-intervaller-melodisk/${String(currentAnswerIndex).padStart(2, '0')}g${currentAnswerSuffix}.opus`;
  }
